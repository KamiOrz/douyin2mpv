...
点击右上角时钟图标查看历史记录，只展示房间 ID 和自行填写的备注。点击 ID 可重新解析并播放；备注直接编辑、自动保存，右键某行可移除记录。成功解析后记录该房间，重复访问会置顶并保留备注。升级时会导入上次输入的有效房间号。

历史记录只在本机 UserDefaults 中保存 ID 与备注，不保存临时直播流地址，也不会上传到仓库。
